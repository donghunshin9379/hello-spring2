	...
	public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
		...
		http.csrf(AbstractHttpConfigurer::disable) // csrf 보안 관련 내용
				.authorizeHttpRequests(authorizeRequest -> authorizeRequest // 권한 부여
						...
						.requestMatchers("/css/**", "/js/**", "/img/**", "/fonts/**", "/", "/login", "/join/**",
								"/home")
						.permitAll() // 전체 권한 가능
						.requestMatchers("/error/**").permitAll() // 에러 권한
						// .requestMatchers("//**").hasAnyRole(new String[]{"ADMIN","USER"}) // 해당 권한만
						// 가능하게
						// .requestMatchers("/admin/**").hasAnyRole(new String[]{"ADMIN"})
						.anyRequest().permitAll() // 지금은 모든 리퀘스트 열려있는 상태 (수정해야됌)
				).formLogin((formLogin) -> formLogin.loginPage("/login") // 로그인 url
						.usernameParameter("userId").passwordParameter("password").loginProcessingUrl("/auth")// 인증절차(로그인
																												// 처리)
																												// url
